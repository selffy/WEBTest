...
chat.ChatModel.prototype = {
   ...
   login: function(nickName) {
      ...
      new ajax.xhr.Request("/WEBTest/jsp/login.jsp", params, 
   ...
   logout: function() {
      ...
      new ajax.xhr.Request("/WEBTest/jsp/logout.jsp", params, 
   ...
   loadMessage: function() {
      ...
      new ajax.xhr.Request("/WEBTest/jsp/loadMessage.jsp", params, 
      ...
      if (req.readyState == 4) {
         if (req.status == 200) {
            ...
            if (code == 'success') {
               ...
               var registerDateTags = xmlDoc.getElementsByTagName("register_date");
               for (var i = 0 ; i < messageTags.length ; i++) {
                  msgs[msgs.length] = messageTags.item(i).firstChild.nodeValue;
                  rdts[rdts.length] = registerDateTags.item(i).firstChild.nodeValue;
               }
               
               this.notify(chat.MESSAGE_LOADED_SUCCESS, msgs, rdts);
               ...
               this.notify(chat.MESSAGE_LOADED_ERROR, "메세지 로드 에러");
            }
         } else {
   ...
   sendMessage: function(msg) {
      ...
      new ajax.xhr.Request("/WEBTest/jsp/sendMessage.jsp", params, 
   ...
   notify: function(type, msgs, rdts) {
      for (var i = 0 ; i < this.listeners.length ; i++) {
         this.listeners[i].actionPerformed(type, msgs, rdts);
      }
   }
}
...
chat.Chat.prototype = {
   ...
   actionPerformed: function(type, msgs, rdts) {
      if (type == chat.LOGIN_SUCCESS) {
         ...
         for (var i = 0 ; i < msgs.length ; i++) {
            this.chatUI.appendData(msgs[i],rdts[i]);
         }
...
chat.ChatUI.prototype = {
   ...
   makeLoginWindow: function() {
      ...
      ajax.Event.addListener(ipBtn, "click",
         ajax.Event.bindAsListener(this.doClickOnBtn, this));
   ...
   doClickOnLoginBtn: function(e) {
      if (this.nickNameInput.value != '') {
         ...
         alert("로그인하실 닉네임을 입력해주세요");
         this.nickNameInput.focus();
      }
   },
   doClickOnIpBtn: function(e) {
      if (this.nickNameInput.value != '') {
         this.chat.login(this.nickNameInput.value);
      } else {
         alert("로그인하실 닉네임을 입력해주세요");
   ...
   appendRegisterDate: function(rdt) {
   this.chatArea.innerHTML += rdt + "<br/>";
   this.chatArea.scrollTop = 
         this.chatArea.scrollHeight-this.chatArea.clientHeight;
   },*/
   appendData: function(rdt, msg) {
   this.chatArea.innerHTML += "[" + rdt + "]" + msg + "<br/>";
   this.chatArea.scrollTop = 
         this.chatArea.scrollHeight-this.chatArea.clientHeight;
   },
   ...
   processError: function(type, errMsg) {
      if (type == chat.LOGIN_ERROR) {
         ...
         alert("메세지 로딩 에러가 발생하였습니다!: "+errMsg);
      } else if (type == chat.MESSAGE_SENDED_SUCCESS) {
